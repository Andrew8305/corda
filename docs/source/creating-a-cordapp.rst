...
Flows are of two types: initiating and initiated. Initiating flows are typically client-side flows and will beused by other
nodes using your CorDapp or users via RPC to initiate communication. These initiation requests trigger the initiated flows
to run. Initiating flows needs to be annotated with ``@InitiatingFlow`` and initiated flows need to be annotated with
...
class with ``CorDappService``. As with the flows, the node will automatically pick this up and create it for use by your
flows. It is recommended that the service class extend ``SingletonSerializeAsToken`` to ensure it doesn't get serialised
in flow checkpoints.
