...
class Cordformation implements Plugin<Project> {
    void apply(Project project) {
        ...
        // Seems to be causing issues with buildCordaJAR?
        project.afterEvaluate {
            project.rootProject.extensions.findByType(Plugin
    ...
    private void configureCordappJar(Project project) {
        ...
        getDirectNonCordaDependencies(project).each {
            println it
        }
        project.tasks.jar.from getDirectNonCordaDependencies(project).collect{ zipTree(it) }.flatten()
        //task createCordappJar(type: Jar, dependsOn: ['classes']) {
        //    from sourceSets.main.output
        //    from getDirectNonCordaDependencies().collect{ zipTree(it) }.flatten()
        //    baseName 'clint-test'
        //}
    }
    ...
    def getDirectNonCordaDependencies(Project project) {
