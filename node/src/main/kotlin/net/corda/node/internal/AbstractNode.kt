...
abstract class AbstractNode(open val configuration: NodeConfiguration,
    ...
    /**
     * Use this method to install your CorDapp services in your tests. This is automatically done by the node when it
     * starts up for all classes it finds which are annotated with [CorDappService].
     */
    ...
    private fun registerInitiatedFlows(scanResult: ScanResult) {
        scanResult.getClassesWithAnnotation(FlowLogic::class, InitiatedBy::class).forEach {
            try {
                ...
                log.warn("Unable to register initiated flow ${it.name}", e)
            }
        }
    }
